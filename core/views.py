...
def index(request):
    ...
    def makeMelody(numNotes):
        ...
        for x in range(0, numNotes):
            choice = random.randint(-8, len(notesString)+7)
            try:
                if choice >= len(notesString):
                    choice = random.randint(melody[x-1-random.randint(0,2)])
                elif (choice < 0):
                    choice = random.randint(melody[x-1+random.randint(0,2)])
            except:
    ...
    def makeTimedMelody(melody, times):
        ...
        for i in range(1, len(melody)):
            if(i == len(melody)):
                song += melody[i][:times[i]].fade_out(3000)
...
def combined(request):
    ...
    def combineSquareRatio(numTunes):
        ...
        request.session['filename'] = filename
        message = "this nice song is generated by taking a weighted random selection of the highest ranked tunes"
